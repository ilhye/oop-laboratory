...
public class Sentence {
    ...
    public static void main(String[] args) {
        ...
        extracted(arrWithoutPunctuation, strSortingOrder);
    }

    private static void extracted(String[] arrWithoutPunctuation, String strSortingOrder) {
        if (strSortingOrder.equalsIgnoreCase("ASC")) { // accepts ESC, esc, Esc,.. input
            for (int i = 1; i < arrWithoutPunctuation.length; i++) {
                ...
                while (intCounterAsc >= 0 && strTemp.length() < arrWithoutPunctuation[intCounterAsc].length()) {
                    arrWithoutPunctuation[intCounterAsc + 1] = arrWithoutPunctuation[intCounterAsc]; // shift element to
                                                                                                     // the right
                    intCounterAsc--;
                }
            ...
            for (int i = 1; i < arrWithoutPunctuation.length; i++) {
                ...
                while (intCounterDes >= 0 && strTemp.length() > arrWithoutPunctuation[intCounterDes].length()) {
                    arrWithoutPunctuation[intCounterDes + 1] = arrWithoutPunctuation[intCounterDes]; // shift element to
                                                                                                     // the right
                    intCounterDes--;
                }
